...
export function getYouTubeClient(credentials, settings) {
...
export async function uploadYouTubeShort(
  ...
  try {
    ...
    console.log(
      `[YouTube API] Preparing video file: ${videoFilePathPlaceholder || "default_shorts_template.mp4"}`,
    );
    console.log(
      `[YouTube API] Embedding subtitle track from generated script: "${scriptText.substring(0, 60)}..."`,
    );
    ...
    const response = await youtube.videos.insert({
      ...
      media: {
        ...
        body:
          videoFilePathPlaceholder &&
          videoFilePathPlaceholder.startsWith("http")
            ? require("stream").Readable.fromWeb(
                (await fetch(videoFilePathPlaceholder)).body,
              )
 ...
 */
export async function postCommunityTab(credentials, captionText, settings) {
  try {
    const youtube = getYouTubeClient(credentials, settings);
    // Validate credentials
    const authClient = youtube.context._options.auth;
    const tokenInfo = await authClient?.getAccessToken();
    if (!tokenInfo) {
      throw new Error(
        "Invalid YouTube Access Token during Community Tab auth check.",
      );
    }

    console.log("[YouTube API] Authenticated successfully.");
    console.log(
      `[YouTube API] Community Tab Post content: "${captionText.substring(0, 80)}..."`,
    );
    ...
    return {
      success: true,
      message:
        "Successfully posted to YouTube Community tab (Simulated OAuth flow)",
    };
  } catch (error) {
