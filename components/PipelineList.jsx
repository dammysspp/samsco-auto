...
export default function PipelineList({ initialItems }) {
  ...
  const [showHistoryItemId, setShowHistoryItemId] = useState(null);
  ...
  const triggerApi = async (url, body, successMessage) => {
    const id = body.id;
  ...
  const regenerateScript = (id) =>
    triggerApi(
      ...
      { id },
      "New script version generated!",
    );

  // 3. Edit & Save Script Changes
  const saveScriptEdits = async (id) => {
    setLoadingItemId(id);
    setApiError(null);
    setSuccessMsg(null);
    try {
  ...
  // 4. Restore Version
  const restoreVersion = async (id, index) => {
    setLoadingItemId(id);
    setApiError(null);
    setSuccessMsg(null);
    try {
  ...
  const approveScript = (id) =>
    triggerApi(
      "/api/script/approve",
      { id },
      "Script approved! Audio rendering initiated.",
    );

  // 6. Retry Audio
  const retryAudio = (id) =>
    triggerApi("/api/audio/retry", { id }, "Retrying Google TTS narration...");
  ...
  const retryRender = (id) =>
    triggerApi(
      "/api/render/retry",
      { id },
      "Retrying Shotstack video render...",
    );
  ...
  const approveVideo = (id) =>
    triggerApi(
      "/api/video/approve",
      { id },
      "Video approved and queued for scheduling.",
    );

  // 10. Return to Script Review
  const returnToScriptReview = async (id) => {
    setLoadingItemId(id);
    setApiError(null);
    setSuccessMsg(null);
    try {
  ...
  const publishNow = (id) =>
    triggerApi(
      "/api/publish",
      { id },
      "Successfully uploaded and published to YouTube Shorts!",
    );
  ...
  const retryPublish = (id) =>
    triggerApi(
      "/api/publish/retry",
      { id },
      "Retrying YouTube Shorts upload...",
    );
  ...
  const getTabItems = () => {
    switch (activeTab) {
      case "drafts":
        return items.filter(
          (item) =>
            item.status === "draft" || item.status === "script_generating",
        );
      case "script_review":
        ...
        {[
          {
            ...
            label: "Drafts",
            count: items.filter(
              (i) => i.status === "draft" || i.status === "script_generating",
            ).length,
          },
          {
            key: "script_review",
            label: "Script Review",
            count: items.filter((i) => i.status === "review_script").length,
          },
          {
            key: "audio_gen",
            label: "Audio Gen",
            count: items.filter(
              (i) =>
                i.status === "audio_generating" || i.status === "audio_failed",
            ).length,
          },
          {
            key: "rendering",
            ...
            count: items.filter(
              (i) =>
                i.status === "rendering_video" || i.status === "render_failed",
            ).length,
          },
          {
            key: "video_review",
            label: "Video Review",
            count: items.filter((i) => i.status === "review_video").length,
          },
          {
            key: "scheduled",
            label: "Scheduled",
            count: items.filter(
              (i) =>
                i.status === "scheduled" ||
                i.status === "publishing" ||
                i.status === "publish_failed",
            ).length,
          },
          {
            key: "published",
            label: "Published",
            count: items.filter((i) => i.status === "posted").length,
          },
        ].map((tab) => (
          <button
            key={tab.key}
      ...
      {filteredItems.length === 0 ? (
        <div className="text-center py-16 border border-dashed border-zinc-850 rounded-xl">
          <p className="text-zinc-500 text-xs">
            No posts currently in this stage.
          </p>
        </div>
      ) : (
          ...
          {filteredItems.map((item) => {
            ...
            try {
              if (item.scriptVersions)
                historyList = JSON.parse(item.scriptVersions);
            } catch (_) {}
            ...
            return (
              ...
              >
                    ...
                    {activeTab === "drafts" && (
                      <>
                        <button
                          ...
                          disabled={
                            isLoading || item.status === "script_generating"
                          }
                            ...
                            {!isRescheduling ? (
                              <button
                                onClick={() => {
                                  ...
                                  setEditScheduledFor(
                                    formatForInput(item.scheduledFor),
                                  );
                                }}
                                className="flex items-center gap-1 px-3 py-2 border border-zinc-700 hover:border-zinc-500 text-zinc-300 text-xs font-semibold rounded-lg bg-zinc-900 transition-all cursor-pointer"
                              >
                              ...
                              <div className="flex items-center gap-1.5">
                                <input
                                  ...
                                  onChange={(e) =>
                                    setEditScheduledFor(e.target.value)
                                  }
                ...
                <div className="flex flex-wrap gap-x-6 gap-y-2 text-[10px] text-zinc-500 pt-2 border-t border-zinc-900">
                  ...
                  {item.scriptGeneratedAt && (
                    <span>
                      Script Gen: {formatDate(item.scriptGeneratedAt)}
                    </span>
                  )}
                  {item.renderCompletedAt && (
                    <span>Rendered: {formatDate(item.renderCompletedAt)}</span>
                  )}
                  {item.scheduledFor && (
                ...
                {activeTab === "script_review" && historyList.length > 1 && (
                  <div className="pt-2">
                    <button
                      onClick={() =>
                        setShowHistoryItemId(isHistoryOpen ? null : item.id)
                      }
                    ...
                    >
                      ...
                      {isHistoryOpen
                        ...
                        : `View Versions (${historyList.length})`}
                    </button>

                    {isHistoryOpen && (
                      <div className="mt-2 p-3 bg-zinc-950 rounded-lg border border-zinc-850 space-y-2 text-xs">
                        <span className="text-zinc-500 font-bold uppercase tracking-wider text-[9px] block">
                          History Tree
                        </span>
                        {historyList.map((hist, idx) => (
                          <div
                            key={idx}
                            className="flex items-center justify-between border-b border-zinc-900 pb-1.5 last:border-0 last:pb-0"
                          >
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4 p-4 rounded-xl border border-zinc-850 bg-zinc-900/10">
                    ...
                    <div className="space-y-2">
                      ...
                      {item.videoUrl ? (
                        ...
                        <span className="text-xs text-zinc-500 italic block pt-1.5">
                          S3 Video compilation not loaded. Check Status.
                        </span>
                      )}
                    </div>
                  </div>
                      ...
                      <div className="p-4 bg-zinc-950 rounded-xl border border-zinc-850 space-y-2">
                        ...
                        {isEditing ? (
                          <textarea
                            ...
                            onChange={(e) =>
                              setEditShortsScript(e.target.value)
                            }
                            rows={8}
                      ...
                      <div className="p-4 bg-zinc-950 rounded-xl border border-zinc-850 space-y-2">
                        ...
                        {isEditing ? (
                          <textarea
                            ...
                            onChange={(e) =>
                              setEditCommunityCaption(e.target.value)
                            }
                            rows={8}
