    ...
    <div className="space-y-8">
      ...
      <div>
        ...
        <p className="text-zinc-400 text-sm mt-1">
          Welcome back, {session?.user?.name || "Football Creator"}. Here is
          your FIFA 2026 World Cup pipeline status.
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4">
        {stats.map((stat) => {
          ...
          return (
            <div
              key={stat.name}
              className={`p-5 rounded-xl border flex flex-col justify-between ${stat.color} bg-zinc-950/60 backdrop-blur-md`}
            >
              ...
              <span className="text-3xl font-bold mt-4 tracking-tight text-zinc-100">
                {stat.value}
              </span>
            </div>
          );
      ...
      <div className="p-6 rounded-xl border border-zinc-800 bg-zinc-950/40 backdrop-blur-sm space-y-4">
        ...
        <p className="text-xs text-zinc-400 max-w-xl leading-relaxed">
          Ingest new RSS articles focusing on the active stages of the FIFA 2026
          World Cup, or force dispatch the scheduled items directly to your
          YouTube Channel.
        </p>
        <OverviewActions />
      </div>
      ...
      <div className="p-6 rounded-xl border border-zinc-800 bg-zinc-950/40 backdrop-blur-sm space-y-4">
        ...
        {upcomingPosts.length === 0 ? (
          <p className="text-sm text-zinc-500 py-4">
            No content scheduled. Ingest news and generate scripts to populate
            the queue.
          </p>
        ) : (
            ...
            <table className="w-full text-left text-sm text-zinc-400">
              ...
              <tbody className="divide-y divide-zinc-800/50">
                {upcomingPosts.map((post) => (
                  ...
                  >
                    <td className="py-3 px-4 font-medium text-zinc-200 max-w-sm truncate">
                      {post.title}
                    </td>
                    <td className="py-3 px-4">
                      {post.scheduledFor
                        ? new Date(post.scheduledFor).toUTCString()
