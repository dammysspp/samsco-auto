...
export async function POST(req) {
  try {
    ...
    if (session?.user) {
      ...
      if (!defaultUser) {
        return NextResponse.json(
          { error: "No user initialized in DB yet." },
          { status: 400 },
        );
      }
    ...
    const targetArticles =
      worldCupArticles.length > 0 ? worldCupArticles : allItems;

    // 4. Sort by date and grab the top 3 newest
    const sorted = targetArticles.sort((a, b) => {
      return (
        new Date(b.pubDate || b.isoDate || 0).getTime() -
        new Date(a.pubDate || a.isoDate || 0).getTime()
      );
    });
