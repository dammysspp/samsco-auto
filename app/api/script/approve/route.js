  ...
  try {
    ...
    if (!item || !item.shortsScript) {
      return NextResponse.json(
        { error: "Content item or script not found." },
        { status: 404 },
      );
    }
    ...
    console.log(
      `[Script Approve] Initializing audio generation for: "${item.title}"`,
    );
    ...
    try {
      console.log(
        `[Script Approve] Triggering Shotstack cloud slideshow render...`,
      );
      ...
      console.error(
        "[Script Approve] Shotstack render trigger failed:",
        compileErr,
      );
      const failedItem = await prisma.contentQueue.update({
        where: { id },
