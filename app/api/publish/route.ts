...
export async function POST(req: Request) {
  try {
    ...
    if (!item.videoUrl) {
      return NextResponse.json({ error: "No compiled video output found. Render must be completed first." }, { status: 400 });
    }
    ...
    if (!settings || !settings.youtubeOAuth) {
      return NextResponse.json({ error: "YouTube integration settings not configured." }, { status: 400 });
    }

    // 1. Lock to publishing
    console.log(`[YouTube Publish] Starting manual publish for: "${item.title}"`);
    ...
    try {
      ...
      console.error("[YouTube Publish] API upload failed: ", uploadErr);
