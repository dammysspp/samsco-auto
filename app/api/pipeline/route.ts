...
export async function PATCH(req: Request) {
  try {
    ...
    const { id, shortsScript, communityCaption, scheduledFor, restoreVersionIndex } = body;
    ...
    // Version restoring
    if (restoreVersionIndex !== undefined && Array.isArray(versionsList) && versionsList[restoreVersionIndex]) {
      ...
      if (communityCaption !== undefined && communityCaption !== item.communityCaption) {
      ...
      if (scriptEdited) {
        versionsList.push({
          timestamp: new Date().toISOString(),
          script: shortsScript !== undefined ? shortsScript : (item.shortsScript || ""),
          caption: communityCaption !== undefined ? communityCaption : (item.communityCaption || ""),
          type: "manual_edit",
        });
        updateFields.scriptVersions = JSON.stringify(versionsList);
      }
