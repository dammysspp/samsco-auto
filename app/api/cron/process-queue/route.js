...
async function handleCronProcess(req) {
  try {
    ...
    for (const item of queueItems) {
      ...
      if (!settings || !settings.youtubeOAuth) {
        await prisma.contentQueue.update({
          ...
          data: {
            ...
            errorMessage:
              "No YouTube OAuth credentials configured in settings.",
          },
        });
        report.push({
          id: item.id,
      ...
      try {
        ...
        await prisma.contentQueue.update({
          ...
          data: {
            status: "publish_failed",
            errorMessage:
              "YouTube OAuth credentials in database are corrupted/invalid JSON.",
          },
        });
        report.push({
          id: item.id,
          status: "failed",
          error: "Corrupted credentials JSON",
        });
        continue;
      }
      ...
      try {
        console.log(
          `[Cron Scheduler] Starting publication for: "${item.title}"`,
        );
        ...
        if (!item.videoUrl) {
          throw new Error(
            "No compiled S3 videoUrl found for this scheduled post.",
          );
        }
        ...
        if (item.communityCaption) {
          try {
            ...
            console.warn(
              "[Cron Scheduler] Community post failed but video succeeded:",
              cErr,
            );
        ...
        await prisma.contentQueue.update({
          ...
          data: {
            ...
            postedAt: now,
            errorMessage: communityMessage.startsWith("Community post warning")
              ? communityMessage
              : null,
          },
        });
        ...
        console.error(
          `[Cron Scheduler] Failed to post item ${item.id}: `,
          uploadError,
        );
        await prisma.contentQueue.update({
          where: { id: item.id },
